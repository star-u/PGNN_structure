...
def train_eval_model(model,
    ...
    #optimizer = optim.Adam([{'params': model.parameters(), 'initial_lr': 1.2*(1e-4)}], lr=1.2*(1e-4))
    #optim.SGD([{'params': model.parameters(), 'initial_lr': 1.2*(1e-4)}], lr=1.2*(1e-4), momentum=cfg.TRAIN.MOMENTUM, nesterov=True)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for inputs in dataloader['train']:
            ...
            with torch.set_grad_enabled(True):
                ...
                s_pred, d_pred = \
                    model(data1, data2, P1_gt, P2_gt, G1_gt, G2_gt, H1_gt, H2_gt, n1_gt, n2_gt, KG, KH, inp_type)#.to(device)
...
if __name__ == '__main__':
    ...
    #optimizer = optim.SGD([{'params': model.parameters(), 'initial_lr': cfg.TRAIN.LR}], lr=cfg.TRAIN.LR, momentum=cfg.TRAIN.MOMENTUM, nesterov=True)
    optimizer = optim.Adam([{'params': model.parameters(), 'initial_lr': cfg.TRAIN.LR}], lr=cfg.TRAIN.LR)
